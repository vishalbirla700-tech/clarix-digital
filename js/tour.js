/* ═══════════════════════════════════════════════════════════
   CLARIX — PRODUCT TOUR  (tour.js)  v20260417G
   - Auto-finishes entire tour after 10s per step (guaranteed)
   - ? button z:10001 always above overlay
   - Double-rAF positioning for accurate spotlight
═══════════════════════════════════════════════════════════ */

const ClarixTour = window.ClarixTour = {

  TOUR_VERSION: '20260417G',
  ...
  TOURS: {
    write: [
      { target:'langBlock',    title:'1 · Choose Your Language', body:'Pick from 20+ Indian & world languages. Clarix will tailor every prompt to your tongue.', arrow:'bottom', hand:'point-down' },
      { target:'modeTabs',     title:'2 · Select a Platform',    body:'Are you writing for an AI tool like ChatGPT, or a social app like Instagram? Pick the right mode.', arrow:'bottom', hand:'point-down' },
      { target:'promptInput',  title:'3 · Type Your Idea',       body:'Write your rough idea in any language — Hindi, Hinglish, English, anything! Even a single line works.', arrow:'right', hand:'point-right' },
      { target:'enhanceBtn',   title:'4 · Hit Enhance ✦',        body:'Tap this button and watch Clarix transform your idea into a powerful, AI-optimised prompt in seconds.', arrow:'top', hand:'tap' },
      { target:'resultsPanel', title:'5 · Your Results Appear Here', body:'Three prompt variations will appear. Pick your favourite, copy it, or open directly in your AI tool!', arrow:'left', hand:'point-left' }
    ],
    inspire: [
      { target:'uploadZone',    title:'1 · Upload Your Photo',   body:'Drag & drop any image, or tap to upload. Gemini Vision will analyse it and generate a perfect prompt.', arrow:'bottom', hand:'point-down' },
      { target:'categoryFilter',title:'2 · Filter by Category',  body:'Switch between Cinematic, Fashion, 3D, Video and more to find prompts that match your creative style.', arrow:'bottom', hand:'point-down' },
      { target:'galleryGrid',   title:'3 · Tap Any Image',       body:'Click any gallery card to open it in the prompt editor — then enhance, copy or send it directly to Write.', arrow:'top', hand:'tap' }
    ]
  },
  ...
  /* ── "?" button — z:10001, always visible above overlay ── */
  _addHelpButton(page) {
    ...
    btn.style.cssText = [
      ...
      'color:#fff','font-size:20px','font-weight:900','border:none',
    ...
    btn.addEventListener('mouseenter', () => { btn.style.transform='scale(1.12)'; btn.style.boxShadow='0 8px 28px rgba(255,112,67,0.7)'; });
    btn.addEventListener('mouseleave', () => { btn.style.transform='scale(1)';    btn.style.boxShadow='0 4px 20px rgba(255,112,67,0.6)'; });
  ...
  _buildDOM() {
      ...
      <div class="ct-tooltip" id="ct-tooltip" role="dialog" aria-live="polite">
        ...
        <div class="ct-tooltip-inner">
          <div class="ct-header">
            ...
            <button class="ct-skip-btn" id="ct-skip-btn" aria-label="Skip tour">Skip</button>
          </div>
    ...
    document.getElementById('ct-next').addEventListener('click',     () => { this._stopAutoTimer(); this._next(); });
    document.getElementById('ct-prev').addEventListener('click',     () => { this._stopAutoTimer(); this._prev(); });
    document.getElementById('ct-skip-btn').addEventListener('click', () => { this._stopAutoTimer(); this.finish(true); });
    this._overlay.addEventListener('click', () => { this._stopAutoTimer(); this._next(); });
  },

  /* ═══════════════════════════════════════════════════════
     _showStep — core logic
     Each step sets a 10-second window-level setTimeout.
     If user does NOT interact, it fires _next() regardless.
     We use window._ctAutoTimer so it survives any context.
  ═══════════════════════════════════════════════════════ */
  _showStep(index) {
    const step = this._steps[index];
    if (!step) { this.finish(false); return; }

    /* ── Kill previous auto-timer before doing anything ── */
    this._stopAutoTimer();
    ...
    /* ── Position spotlight after scroll settles (double rAF) ── */
    if (target) {
    ...
    /* ── 10-second countdown displayed in Next button ── */
    let secs = 10;
    const tick = () => {
      const btn = document.getElementById('ct-next');
      if (btn) btn.textContent = isLast ? `🚀 Get Started! (${secs}s)` : `Next → (${secs}s)`;
      secs--;
      if (secs < 0) {
        /* Time's up — force advance / finish */
        this._stopAutoTimer();
        this._next();
        return;
      }
      window._ctAutoTimer = setTimeout(tick, 1000);
    };
    /* Start after 1s so first render shows clean text briefly */
    window._ctAutoTimer = setTimeout(tick, 1000);
  },

  /* ── Stop window-level timer ── */
  _stopAutoTimer() {
    if (window._ctAutoTimer) {
      clearTimeout(window._ctAutoTimer);
      window._ctAutoTimer = null;
    }
  },
  ...
  _bindKeys() {
    this._keyHandler = (e) => {
      if (e.key === 'ArrowRight' || e.key === 'Enter') { this._stopAutoTimer(); this._next(); }
      if (e.key === 'ArrowLeft')  { this._stopAutoTimer(); this._prev(); }
      if (e.key === 'Escape')     { this._stopAutoTimer(); this.finish(true); }
    };
